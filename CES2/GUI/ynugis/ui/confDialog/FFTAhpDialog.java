...
public class FFTAhpDialog extends Dialog {
	...
	private Button simpleRdo;

	private Button complexRdo;
	...
	protected Control createDialogArea(Composite parent) {
		...
		final Group typeGroup = new Group(content, SWT.NONE);
		{
			typeGroup.setLayoutData(new GridData(GridData.BEGINNING,
					GridData.CENTER, false, false, 50, 1));
			typeGroup.setText("选择评分方式");
		...
		simpleRdo = new Button(typeGroup, SWT.RADIO);
		{
			GridData grid = new GridData(GridData.FILL_HORIZONTAL);
			grid.horizontalAlignment = SWT.CENTER;
			simpleRdo.setLayoutData(grid);
			simpleRdo.setSelection(true);
			simpleRdo.setText("简化");
		}

		complexRdo = new Button(typeGroup, SWT.RADIO);
		{
			complexRdo.setText("完全");
			GridData grid = new GridData(GridData.FILL_HORIZONTAL);
			grid.horizontalAlignment = SWT.CENTER;
			complexRdo.setLayoutData(grid);
		}
			...
			public void mouseUp(MouseEvent e) {
				if (e.button > 1) {
					...
					if (objTree != null) {
						...
						new FFTAhpMatrixDialog(treeItem, tree2FFT, isSimpleAhp())
								.open();
						if (fftList.getSelectionCount() != 1)
							return;
						String sled = fftList.getSelection()[0];
	...
	private boolean isSimpleAhp() {
		if (simpleRdo.getSelection())
			return true;
		else
			return false;
	}
